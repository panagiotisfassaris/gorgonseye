...
def encrypt_help():
    ...
    print("  -r          Encrypt files recursively in the specified directory (leave empty to use current)")
...
def decrypt_help():
    ...
    print("  -r          Decrypt files recursively in the specified directory (leave empty to use current)")
    print("  --help      Show this help message")
...
def encrypt_directory(directory, public_key):
    exclude_files = ['key.bin', 'private_key.pem', 'public_key.pem', 
                     'encrypt.py', 'decrypt.py', 'encrypt.exe', 
                     'decrypt.exe', 'enc_header.py', 'enc_header.cpython-311.pyc']
...
def DecryptFile(file, key):
    try:
        ...
        output_file = file.rsplit('.', 1)[0]  # Removes .enc
        with open(output_file, 'wb') as output_file:
    ...
    try:
        for root, dirs, files in os.walk(wd):
            for name in files:
                ...
                if name not in ['key.bin', 'private_key.pem', 'public_key.pem', 'encrypt.py', 'decrypt.py', 'encrypt.exe', 'decrypt.exe', 'enc_header.py', 'enc_header.cpython-311.pyc']:
